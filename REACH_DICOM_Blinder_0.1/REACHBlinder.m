function REACHBlinder(hObject, handles, AFFoutput, AFFheader, folder_name2, blinding_id, blinding_acrostic, blindflag)
...
col_SOPInstanceUID =    indcfind(AFFheader','^SOPInstanceUID$','regexpi');
...
anon_keep = {'PatientID','PatientName','StudyDate','StudyTime','StudyDescription','SeriesDescription','StudyInstanceUID','SeriesInstanceUID','SOPInstanceUID'};
...
for ix=2:size(AFFoutput,1) %loop through files
    ...
    if(strcmpi('yes',tmp_isdcm)) %continue if DICOM file
        ...
        tmp_SOPInstanceUID = AFFoutput{ix,col_SOPInstanceUID};
...
%                     blindqualifier = 1;
%                 end
                if(~strcmpi('.',tmp_PatientID) && ismember(tmp_Modality,Modality_WhiteList))
                    %continue if PatientID and Modality is valid format
                    blindqualifier = 1;
                end
...
%                     end
                    ...
                    new_ID = upper(blinding_id);
                    new_Name = horzcat(upper(blinding_acrostic),'^^^^');
                    
                    datequalifier = 1; %do not limit blinding on date
...
%                 new_uid_instance =  dicomuid;
                ...
                if(~strcmpi(strtrim(tmp_MediaStorage),'1.2.840.10008.5.1.4.1.1.7')) %file MediaStorageSOPClassUID is NOT SecondaryCaptureImageStorage
                    tmpf_out = horzcat(tmpf_out,new_ID,'\',tmp_StudyDate,'\',tmp_SeriesUID,'\',tmp_SOPInstanceUID,'.dcm');
                else
                    tmpf_out = horzcat(tmpf_out,'SecondaryCaptureImages\',new_ID,'\',tmp_StudyDate,'\',tmp_SeriesUID,'\',tmp_SOPInstanceUID,'.dcm');
                end
                ...
                switch blindflag
                    ...
                    case 1
                        %%
                        try
                            dicomanon(tmpf_out,tmpf_out,'keep',anon_keep,'UseVRHeuristic',false);
                        catch anon_err
                            anon_flag = 1;
                        end
